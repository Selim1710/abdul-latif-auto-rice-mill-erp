          ...
          @csrf
            ...
            <div class="modal-body">
                <div class="row">
                    ...
                    <x-form.textbox labelName="{{__('file.Name')}}" name="name" required="required" col="col-md-6" placeholder="{{__('file.Name')}}"/>
                    <x-form.textbox labelName="{{__('file.Username')}}" name="username" required="required" col="col-md-6" placeholder="{{__('file.Username')}}"/>
                    <x-form.textbox labelName="{{__('file.Phone No')}}." name="phone" required="required" col="col-md-6" placeholder="{{__('file.Phone No')}}"/>
                    <x-form.textbox labelName="{{__('file.Email')}}" name="email" col="col-md-6" placeholder="Enter email"/>
                    <x-form.selectbox labelName="{{__('file.Gender')}}" name="gender" required="required" col="col-md-6" class="selectpicker">
                        <option value="1">Male</option>
                        <option value="2">Female</option>
                        <option value="3">Other</option>
                    </x-form.selectbox>
                    <x-form.selectbox labelName="{{__('file.Role')}}" name="role_id" required="required" col="col-md-6" class="selectpicker">
                        @foreach ($roles as $role)
                            <option value="{{ $role->id }}">{{ $role->role_name }}</option>
                        @endforeach
                    </x-form.selectbox>
                    <div class="col-md-6 form-group">
                        <label for="site_title">{{__('file.Password')}}</label>
                    ...
                    <div class="col-md-6 form-group">
                        <label for="site_title">{{__('file.Confirm Password')}}</label>
                        <div class="input-group">
