...
class UnitController extends BaseController{
    ...
    public function index(){
        if(permission('unit-access')){
            $setTitle = __('file.Unit');
            $this->setPageData($setTitle,$setTitle,'fas fa-balance-scale',[['name' => $setTitle]]);
    ...
    public function get_datatable_data(Request $request){
        if($request->ajax() && permission('unit-access')){
            ...
            foreach ($list as $value) {
                ...
                if(permission('unit-delete')){
                    $action .= ' <a class="dropdown-item delete_data"  data-id="' . $value->id . '" data-name="' . $value->unit_name . '">'.$this->actionButton('Delete').'</a>';
                }
                $row     = [];
                $row[]   = $no;
                $row[]   = $value->unit_name;
    ...
    public function store_or_update_data(UnitFormRequest $request){
        if($request->ajax() && permission('unit-add')){
