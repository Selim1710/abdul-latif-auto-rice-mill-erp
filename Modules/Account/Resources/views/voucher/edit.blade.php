            ...
            <div class="card card-custom gutter-b">
                <div class="card-header flex-wrap py-5">
                    ...
                    @if (permission('voucher-access'))
                        <div class="card-toolbar"><a href="{{ route('voucher') }}"
                        ...
                        <div id="kt_datatable_wrapper" class="dataTables_wrapper dt-bootstrap4 no-footer">
                            <div class="row">
                                ...
                                <div class="form-group col-md-4 required">
                                    ...
                                    <input type="text" class="form-control bg-primary" id="voucher_no" name="voucher_no"
                                        value="{{ $voucherData[0]->voucher_no }}" readonly />
                                </div>
                                <div class="form-group col-md-4 required">
                                    <label for="voucher_date"> {{ __('file.Date') }}</label>
                                    <input type="date" class="form-control" name="date" id=date"
                                        value="{{ $voucherData[0]->date }}" />
                                </div>
                                <div class="form-group col-md-4 required">
                                    <label for="voucher_type"> {{ __('file.Voucher Type') }}</label>
                                    <select class="form-control voucher_type selectpicker" id="voucher_type"
                                        name="voucher_type" required>
                                        <option value="">Select Please</option>
                                        @foreach (VOUCHER_TYPE_VALUE as $key => $type)
                                            <option value="{{ $key }}"
                                                @if ($type == $voucherData[0]->voucher_type) selected="selected" @endif>
                                                {{ $type }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            ...
                            <div class="row">
                                <div class="col-md-6">
                                    <table class="table table-bordered" id="debit-table">
                                        ...
                                        <tbody>
                                            @foreach ($voucherData as $key => $debit)
                                                @if ($debit->debit > 0)
                                                    <tr class="text-center">
                                                        <td>
                                                            <button type="button" class="btn btn-primary btn-sm add-more"
                                                                id="add_more" data-type="debit-add" data-toggle="tooltip"
                                                                data-placement="top" data-original-title="Add More"><i
                                                                    class="fas fa-plus-square"></i></button>
                                                            <button type="button" class="btn btn-danger btn-sm remove"
                                                                data-type="debit-remove" data-toggle="tooltip"
                                                                data-placement="top" data-original-title="Remove"
                                                                style="margin-top:3px"> <i
                                                                    class="fas fa-minus-square"></i></button>
                                                        </td>
                                                        <td>
                                                            <select name="debit[{{ $key }}][chart_of_head_id]"
                                                                id="debit_{{ $key }}_chart_of_head_id"
                                                                class="form-control selectpicker" data-live-search="true">
                                                                <option value="">Select Please</option>
                                                                @foreach ($heads as $head)
                                                                    <option value="{{ $head->id }}"
                                                                        @if ($head->id == $debit->chart_of_head_id) selected="selected" @endif>
                                                                        {!! $head->name !!}</option>
                                                                @endforeach
                                                                @foreach ($subHeads as $subHead)
                                                                    <option value="{{ $subHead->id }}"
                                                                        @if ($subHead->id == $debit->chart_of_head_id) selected="selected" @endif>
                                                                        {!! ($subHead->head->name ?? '') . '  ---  ' . ($subHead->name ?? '') !!}</option>
                                                                @endforeach
                                                                @foreach ($childHeads as $child)
                                                                    <option value="{{ $child->id }}"
                                                                        @if ($child->id == $debit->chart_of_head_id) selected="selected" @endif>
                                                                        {!! ($child->head->name ?? '') . '  ---  ' . ($child->subHead->name ?? '') . '  ---  ' . ($child->name ?? '') !!}</option>
                                                                @endforeach
                                                            </select>
                                                        </td>
                                                        <td>
                                                            <input type="text" class="form-control debitAmount onInput"
                                                                name="debit[{{ $key }}][debit]"
                                                                id="debit_{{ $key }}_debit"
                                                                value="{{ $debit->debit }}" />
                                                        </td>
                                                    </tr>
                                                @endif
                                        ...
                                        <tfoot>
                                            <tr>
                                                ...
                                                <td><input type="text" class="form-control text-right bg-primary"
                                                        name="total_debit_amount" id="total_debit_amount" placeholder="0.00"
                                                        readonly></td>
                                            </tr>
                                        </tfoot>
                                    </table>
                                ...
                                <div class="col-md-6">
                                    <table class="table table-bordered" id="credit-table">
                                        ...
                                        <tbody>
                                            @foreach ($voucherData as $key => $credit)
                                                @if ($credit->credit > 0)
                                                    <tr class="text-center">
                                                        <td>
                                                            <input type="text" class="form-control creditAmount onInput"
                                                                name="credit[{{ $key }}][credit]"
                                                                id="credit_{{ $key }}_credit"
                                                                value="{{ $credit->credit }}" />
                                                        </td>
                                                        <td>
                                                            <select class="form-control selectpicker"
                                                                id="credit_{{ $key }}_chart_of_head_id"
                                                                name="credit[{{ $key }}][chart_of_head_id]"
                                                                data-live-search="true">
                                                                <option value="">Select Please</option>
                                                                @foreach ($heads as $head)
                                                                    <option value="{{ $head->id }}"
                                                                        @if ($head->id == $credit->chart_of_head_id) selected="selected" @endif>
                                                                        {!! $head->name !!}</option>
                                                                @endforeach
                                                                @foreach ($subHeads as $subHead)
                                                                    <option value="{{ $subHead->id }}"
                                                                        @if ($subHead->id == $credit->chart_of_head_id) selected="selected" @endif>
                                                                        {!! ($subHead->head->name ?? '') . '  ---  ' . $subHead->name !!}</option>
                                                                @endforeach
                                                                @foreach ($childHeads as $child)
                                                                    <option value="{{ $child->id }}"
                                                                        @if ($child->id == $credit->chart_of_head_id) selected="selected" @endif>
                                                                        {!! ($child->head->name ?? '') . '  ---  ' . ($child->subHead->name ?? '') . '  ---  ' . ($child->name ?? '') !!}</option>
                                                                @endforeach
                                                            </select>
                                                        </td>
                                                        <td>
                                                            <button type="button" class="btn btn-primary btn-sm add-more"
                                                                id="credit-add" data-type="credit" data-toggle="tooltip"
                                                                data-placement="top" data-original-title="Add More"><i
                                                                    class="fas fa-plus-square"></i></button>
                                                            <button type="button" class="btn btn-danger btn-sm remove"
                                                                data-type="credit-remove" data-toggle="tooltip"
                                                                data-placement="top" data-original-title="Remove"
                                                                style="margin-top:3px"> <i
                                                                    class="fas fa-minus-square"></i></button>
                                                        </td>
                                        ...
                                        <tfoot>
                                            <tr>
                                                <td><input type="text" class="form-control text-right bg-primary"
                                                        name="total_credit_amount" id="total_credit_amount"
            ...
            if (type == 'debit-add') {
                if ($('.creditAmount').length == 1) {
                    html = `<tr class="text-center">
                             ...
                             <td>
                                <input type="text" class="form-control debitAmount onInput" name="debit[` + i +
                        `][debit]" id="debit_` + i + `_debit"/>
                             </td>
                          </tr>`
                ...
                if ($('.debitAmount').length == 1) {
                    html = `<tr class="text-center">
                             <td>
                               <input type="text" class="form-control creditAmount onInput" name="credit[` + i + `][credit]" id="credit_{{ $key }}_credit"/>
                             </td>
                             <td>
                               <select class="form-control selectpicker" id="credit_` + i +
                        `_chart_of_head_id" name="credit[` + i + `][chart_of_head_id]" data-live-search="true">
                               <option value="">Select Please</option>
                               @foreach ($heads as $head)
                               <option value="{{ $head->id }}">{!! $head->name !!}</option>
                               @endforeach
                ...
                success: function(data) {
                    ...
                    if (data.status == false) {
                        $.each(data.errors, function(key, value) {
                            ...
                            $('#voucher_form #' + key).parent().append(
                                '<small class="error text-danger">' + value + '</small>');
                        });
                    } else {
                        notification(data.status, data.message);
                        if (data.status == 'success') {
                            window.location.replace("{{ url('voucher') }}");
                        }
                    }
                },
