            ...
            <form id="voucher_form" method="post">
                        ...
                        <div id="kt_datatable_wrapper" class="dataTables_wrapper dt-bootstrap4 no-footer">
                            <div class="row">
                                ...
                                <div class="form-group col-md-4 required">
                                    ...
                                    <input type="date" class="form-control" name="date" id=date" value="{{$voucherData[0]->date}}"/>
                                </div>
                                <div class="form-group col-md-4 required">
                                    ...
                                    <select class="form-control voucher_type selectpicker" id="voucher_type" name="voucher_type" required >
                                        ...
                                        @foreach (VOUCHER_TYPE_VALUE as $key=>$type)
                                            <option value="{{$key }}" @if($type == $voucherData[0]->voucher_type) selected="selected" @endif>{{ $type }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                            ...
                            <div class="row">
                                <div class="col-md-6">
                                    <table class="table table-bordered" id="debit-table">
                                        ...
                                        <tr class="text-center">
                                            ...
                                            <td>
                                                <select name="debit[{{$key}}][chart_of_head_id]" id="debit_{{$key}}_chart_of_head_id" class="form-control selectpicker" data-live-search="true">
                                                    <option value="">Select Please</option>
                                                    @foreach($heads as $head)
                                                        <option value="{{$head->id}}" @if($head->id == $debit->chart_of_head_id) selected="selected" @endif>{!! $head->name !!}</option>
                                                    @endforeach
                                                    @foreach($subHeads as $subHead)
                                                        <option value="{{$subHead->id}}" @if($subHead->id == $debit->chart_of_head_id) selected="selected" @endif>{!! $subHead->head->name.'  ---  '.$subHead->name !!}</option>
                                                    @endforeach
                                                    @foreach($childHeads as $child)
                                                        <option value="{{$child->id}}" @if($child->id == $debit->chart_of_head_id) selected="selected" @endif>{!! $child->head->name.'  ---  '.$child->subHead->name.'  ---  '.$child->name !!}</option>
                                                    @endforeach
                                                </select>
                                            </td>
                                            <td>
                                                <input type="text" class="form-control debitAmount onInput" name="debit[{{$key}}][debit]" id="debit_{{$key}}_debit" value="{{$debit->debit}}"/>
                                            </td>
                                        </tr>
                                            @endif
                                        @endforeach
                                ...
                                <div class="col-md-6">
                                    <table class="table table-bordered" id="credit-table">
                                        ...
                                        <tr class="text-center">
                                            <td>
                                                <input type="text" class="form-control creditAmount onInput" name="credit[{{$key}}][credit]" id="credit_{{$key}}_credit" value="{{$credit->credit}}"/>
                                            </td>
                                            <td>
                                                <select class="form-control selectpicker" id="credit_{{$key}}_chart_of_head_id" name="credit[{{$key}}][chart_of_head_id]" data-live-search="true">
                                                    <option value="">Select Please</option>
                                                    @foreach($heads as $head)
                                                        <option value="{{$head->id}}" @if($head->id == $credit->chart_of_head_id) selected="selected" @endif>{!! $head->name !!}</option>
                                                    @endforeach
                                                    @foreach($subHeads as $subHead)
                                                        <option value="{{$subHead->id}}" @if($subHead->id == $credit->chart_of_head_id) selected="selected" @endif>{!! $subHead->head->name.'  ---  '.$subHead->name !!}</option>
                                                    @endforeach
                                                    @foreach($childHeads as $child)
                                                        <option value="{{$child->id}}" @if($child->id == $credit->chart_of_head_id) selected="selected" @endif>{!! $child->head->name.'  ---  '.$child->subHead->name.'  ---  '.$child->name !!}</option>
                                                    @endforeach
                                                </select>
                                            </td>
                                            <td>
                ...
                <div class="card card-custom">
                    <div class="card-body">
                        <div id="kt_datatable_wrapper" class="dataTables_wrapper dt-bootstrap4 no-footer">
                            <div class="row">
                                <x-form.textarea labelName="Narration" name="narration" col="col-md-12" value="{{$voucherData[0]->narration}}"/>
                            </div>
                        </div>
                    </div>
                </div>
