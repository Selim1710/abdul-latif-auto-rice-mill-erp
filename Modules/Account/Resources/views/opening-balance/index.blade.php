                ...
                <div class="card-header flex-wrap py-5">
                    ...
                    <div class="card-toolbar">
                        @if (permission('opening-balance-add'))
                            <a href="{{ route('opening.balance.create') }}"  class="btn btn-primary btn-sm font-weight-bolder"><i class="fas fa-plus-circle"></i> Add New</a>
                        @endif
                    </div>
                </div>
            </div>
                                ...
                                <div class="table-responsive">
                                    <table id="dataTable" class="table table-bordered table-hover">
                                        ...
                                        <tr>
                                            ...
                                            <th width="5%">{{__('file.Date')}}</th>
                                            <th>{{__('file.Voucher No')}}</th>
                                            <th>{{__('file.Voucher Type')}}</th>
                                            <th>{{__('file.Description')}}</th>
                                            <th width="5%">{{__('file.Debit')}}</th>
                                            <th width="5%">{{__('file.Credit')}}</th>
                                            <th width="5%">{{__('file.Status')}}</th>
                                            <th width="5%">{{__('file.Created By')}}</th>
                                            <th width="5%">{{__('file.Action')}}</th>
                                        </tr>
                                        </thead>
                                        <tbody></tbody>
                                        <tfoot>
    ...
    <script>
        ...
        $(document).ready(function(){
            ...
            table = $('#dataTable').DataTable({
                ...
                "ajax": {
                    "url" : "{{route('opening.balance.datatable.data')}}",
                    "type": "POST",
                    "data": function (data) {
            ...
            $(document).on('click', '.change_status', function () {
                ...
                let url    = "{{ route('opening.balance.change.status') }}";
            ...
            $(document).on('click', '.delete_data', function () {
                ...
                let name  = $(this).data('name');
                let row   = table.row($(this).parent('tr'));
                let url   = "{{ route('opening.balance.delete') }}";
