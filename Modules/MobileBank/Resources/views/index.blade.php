        ...
        <div class="card card-custom gutter-b">
            <div class="card-header flex-wrap py-5">
                ...
                <div class="card-toolbar">
                    ...
                    <a href="javascript:void(0);" onclick="showFormModal('{{__('file.Add New Mobile Bank')}}','{{__('file.Save')}}')" class="btn btn-primary btn-sm font-weight-bolder"><i class="fas fa-plus-circle"></i>  {{__('file.Add New')}}</a>
                   @endif
                </div>
            </div>
        </div>
        <div class="card card-custom">
            <div class="card-header flex-wrap py-5">
                <form method="POST" id="form-filter" class="col-md-12 px-0">
                    <div class="row">
                        <x-form.textbox labelName="{{__('file.Mobile Bank Name')}}" name="bank_name" col="col-md-3" />
                        <x-form.textbox labelName="{{__('file.Account Name')}}" name="account_name" col="col-md-3" />
                        <x-form.textbox labelName="{{__('file.Account Number')}}" name="account_number" col="col-md-3" />
                        <div class="col-md-3">
                        ...
                        <div class="col-sm-12">
                            <table id="dataTable" class="table table-bordered table-hover">
                                <thead class="bg-primary">
                                    <tr>
                                        <th>{{__('file.SL')}}</th>
                                        <th>{{__('file.Bank Name')}}</th>
                                        <th>{{__('file.Account Name')}}</th>
                                        <th>{{__('file.Account Number')}}</th>
...
<script>
    ...
    $(document).ready(function(){
        table = $('#dataTable').DataTable({
            ...
            "ajax": {
                "url": "{{route('mobilebank.datatable.data')}}",
                "type": "POST",
                "data": function (data) {
        ...
        $(document).on('click', '#save-btn', function () {
            ...
            let url      = "{{route('mobilebank.store.or.update')}}";
                ...
                $.ajax({
                    ...
                    success : function (data) {
                        if(data.status == 'error'){
                            ...
                            $('#store_or_update_modal .modal-title').html('<i class="fas fa-edit text-white"></i> <span>{{__('file.Edit')}} ' + data.bank_name + '</span>');
        ...
        $(document).on('click', '.change_status', function () {
            ...
            let url    = "{{ route('mobilebank.change.status') }}";
        ...
        $(document).on('click', '.delete_data', function () {
            ...
            let name  = $(this).data('name');
            let row   = table.row($(this).parent('tr'));
            let url   = "{{ route('mobilebank.delete') }}";
