...
class TenantReturnProductController extends BaseController
{
    ...
    public function returnData(Request $request)
    {
        ...
        if ($tenant_id) {
            $data['tenant_warehouse_products'] =  TenantWarehouseProduct::where(['tenant_id' => $tenant_id, 'tenant_product_type' => 1])->get();
            // return $data['tenant_warehouse_products'];

            return view('tenant::tenantReturn.data', $data);
        }
    }
            ...
            try {
                ...
                foreach ($tenantReturn->tenantReturnProductList as $value) {
                    $tenantWarehouseProduct = TenantWarehouseProduct::firstWhere([
                        'tenant_id' => $tenantReturn->tenant_id,
                        'warehouse_id' => $value->warehouse_id,
                        'batch_no' => $value->batch_no,
                        'product_id' => $value->product_id,
