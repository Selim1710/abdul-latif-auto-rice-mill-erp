            ...
            <div class="card card-custom gutter-b">
                <div class="card-header flex-wrap py-5">
                    ...
                    @if(permission('tenant-receive-access'))
                        <div class="card-toolbar"><a href="{{ route('tenant.receive.product') }}" class="btn btn-warning btn-sm font-weight-bolder"><i class="fas fa-arrow-left"></i> {{__('file.Back')}}</a></div>
                    @endif
                </div>
            </div>
                ...
                <div class="card-body">
                    <div id="kt_datatable_wrapper" class="dataTables_wrapper dt-bootstrap4 no-footer">
                        <form action="" id="tenant_receive_product_form" method="post" enctype="multipart/form-data">
                            @csrf
                            <input type="hidden" name="update_id" value="{{$tenantReceive->id}}"/>
                            <div class="row">
                                ...
                                <div class="form-group col-md-4 required">
                                    ...
                                    <input type="date" class="form-control date" id="date" name="date" value="{{$tenantReceive->date}}"/>
                                </div>
                                <div class="form-group col-md-4 required">
                                    ...
                                    <select class="form-control selectpicker" id="tenant_id" name="tenant_id" data-live-search = "true">
                                        ...
                                        @foreach($tenants as $tenant)
                                            <option value="{{$tenant->id}}" @if($tenant->id == $tenantReceive->tenant_id) selected="selected" @endif>{{$tenant->name.'('.$tenant->mobile.')'}}</option>
                                        @endforeach
                                    </select>
                                </div>
                                <div class="col-md-12">
                                ...
                                <div class="col-md-12">
                                    <div class="form-group">
                                        <table class="table" id="tenantReceiveTable">
                                            ...
                                            <tbody>
                                            @if(isset($tenantReceive->tenantReceiveProductList))
                                            @foreach($tenantReceive->tenantReceiveProductList as $key => $item)
                                            <tr class="text-center">
                                                <td>
                                                    <select class="form-control selectpicker text-center" id="tenant_receive_{{$key}}_warehouse_id" name="tenant_receive[{{$key}}][warehouse_id]" data-live-search = "true">
                                                        <option value = "{{$item->warehouse_id}}">{{$item->warehouse->name}}</option>
                                                    </select>
                                                </td>
                                                <td>
                                                    <select class="form-control selectpicker category text-center" id="tenant_receive_{{$key}}_category_id" data-product_id="tenant_receive_{{$key}}_product_id" data-live-search = "true">
                                                        <option value = "{{$item->product->category_id}}">{{$item->product->category->category_name}}</option>
                                                    </select>
                                                </td>
                                                <td>
                                                    <select class="form-control selectpicker product text-center" id="tenant_receive_{{$key}}_product_id" name="tenant_receive[{{$key}}][product_id]" data-unit_show="tenant_receive_{{$key}}_unit_show" data-unit_id="tenant_receive_{{$key}}_unit_id"  data-live-search = "true">
                                ...
                                <div class="col-md-12">
                                    ...
                                    <textarea class="form-control" id="note" name="note">{{$tenantReceive->note}}</textarea>
                                </div>
...
@push('scripts')
    <script>
        let i = {{count($tenantReceive->tenantReceiveProductList)}} + 1;
        function _(x){
        ...
        $(document).on('click','.addRaw',function(){
            ...
            html = `<tr class="text-center">
                      <td>
                      <select class="form-control selectpicker text-center" id="tenant_receive_`+ i +`_warehouse_id" name="tenant_receive[`+ i +`][warehouse_id]" data-live-search = "true">
                      ...
                      <td>
                      <select class="form-control selectpicker category text-center" id="tenant_receive_`+ i +`_category_id" data-product_id="tenant_receive_`+ i +`_product_id" data-live-search = "true">
                      <option value="">{{__('Please Select')}}</option>
        ...
        function updateData(){
            ...
            let url      = "{{route('tenant.receive.product.update')}}";
            $.ajax({
                url           : url,
                type          : "POST",
                data          : formData,
                ...
                success       : function (data) {
                    ...
                    if (data.status == false) {
                        $.each(data.errors, function (key, value) {
                            ...
                            $('#tenant_receive_product_form select#' + key).parent().addClass('is-invalid');
                            $('#tenant_receive_product_form #' + key).parent().append('<small class="error text-danger">' + value + '</small>');
                        });
                    } else {
                        notification(data.status, data.message);
                        if (data.status == 'success') { window.location.replace("{{ route('tenant.receive.product') }}"); }
                    }
                },
