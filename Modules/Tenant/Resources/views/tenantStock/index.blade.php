            ...
            <div class="card card-custom">
                <div class="card-header flex-wrap py-5">
                    <form method="POST" id="form-filter" class="col-md-12 px-0">
                        <div class="row">
                            <x-form.selectbox labelName="{{__('file.Tenant')}}" name="tenant_id" col="col-md-4" class="selectpicker">
                                <option value="0" selected>{{__('file.Please Select')}}</option>
                                @foreach ($tenants as $tenant)
                                    <option value="{{ $tenant->id }}">{{ $tenant->name }}</option>
                                @endforeach
                            </x-form.selectbox>
                            <x-form.selectbox labelName="{{__('file.Company')}}" name="warehouse_id" col="col-md-4" class="selectpicker">
                                <option value="0" selected>{{__('file.All Company')}}</option>
                                @foreach ($warehouses as $warehouse)
                                    <option value="{{ $warehouse->id }}">{{ $warehouse->name }}</option>
                                @endforeach
                            </x-form.selectbox>
                            <x-form.selectbox labelName="{{__('file.Product')}}" name="product_id" col="col-md-4" class="selectpicker">
                        ...
                        <div class="row">
                            <div class="col-sm-12">
                                <table id="dataTable" class="table table-bordered table-hover">
                                    ...
                                    <tr class="bg-primary">
                                        <th colspan="7"></th>
                                        <th style="text-align: right !important;font-weight:bold;color:white;">{{__('file.Total')}}</th>
