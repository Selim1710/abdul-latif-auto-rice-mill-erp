            ...
            <div class="card card-custom gutter-b">
                <div class="card-header flex-wrap py-5">
                    ...
                    @if (permission('tenant-return-access'))
                        <div class="card-toolbar"><a href="{{ route('tenant.return.product') }}"
                                    ...
                                    <select class="form-control selectpicker" id="tenant_id" name="tenant_id"
                                        ...
                                        @foreach ($tenants as $tenant)
                                            <option value="{{ $tenant->id }}"
                                                @if ($tenant->id == $tenantReturn->tenant_id) selected="selected" @endif>
                                                        ...
                                                        <tr class="text-center">
                                                            <td>
                                                                <select class="form-control selectpicker text-center"
                                                                    id="tenant_return_{{ $key }}_warehouse_id"
                                                                    name="tenant_return[{{ $key }}][warehouse_id]"
                                                                    data-live-search = "true">
                                                            ...
                                                            <td>
                                                                <input class="form-control bg-primary text-center"
                                                                    name="tenant_return[{{ $key }}][batch_no]"
                                                                    id="tenant_return_{{ $key }}_batch_no"
                                                                    value="{{ $item->batch_no ?? '' }}"
                                                                    readonly />
                                                            </td>
                                                            <td>
                                                                <select
                                                                    ...
                                                                    id="tenant_return_{{ $key }}_product_id"
                                                                    name="tenant_return[{{ $key }}][product_id]"
                                                                    data-warehouse_id = "tenant_return_{{ $key }}_warehouse_id"
                                                                    data-unit_show="tenant_return_{{ $key }}_unit_show"
                                                                    data-unit_id="tenant_return_{{ $key }}_unit_id"
                                                                    data-av_qty="tenant_return_{{ $key }}_av_qty"
                                                                    data-av_scale="tenant_return_{{ $key }}_av_scale"
                                                                    data-live-search = "true">
                                                            ...
                                                            <td><input class="form-control av_scale text-center bg-primary"
                                                                    id="tenant_return_{{ $key }}_av_scale"
                                                                    value="{{ $item->availableQty($item->warehouse_id, $item->product_id)->scale ?? 0 }}"
                                                                    readonly /></td>
                                                            <td><input class="form-control qty text-center"
                                                                    id="tenant_return_{{ $key }}_qty"
                                                                    name="tenant_return[{{ $key }}][qty]"
                                                                    value="{{ $item->qty }}"
                                                                    data-product_id="tenant_return_{{ $key }}_product_id"
                                                                    data-unit_id="tenant_return_{{ $key }}_unit_id"
                                                                    data-av_qty="tenant_return_{{ $key }}_av_qty"
                                                                    data-av_scale="tenant_return_{{ $key }}_av_scale"
                                                                    data-scale="tenant_return_{{ $key }}_scale" />
                                                            </td>
                                                            <td><input class="form-control scale text-center"
                                                                    id="tenant_return_{{ $key }}_scale"
                                                                    name="tenant_return[{{ $key }}][scale]"
                                                                    value="{{ $item->scale }}"
                                                                    data-product_id="tenant_return_{{ $key }}_product_id"
                                                                    data-unit_id="tenant_return_{{ $key }}_unit_id"
                                                                    data-av_qty="tenant_return_{{ $key }}_av_qty"
                                                                    data-av_scale="tenant_return_{{ $key }}_av_scale"
                                                                    data-qty="tenant_return_{{ $key }}_qty" />
                                                            </td>
                                                            <td><input class="form-control retQty"
                                                                    id="tenant_return_{{ $key }}_ret_qty"
                                                                    name="tenant_return[{{ $key }}][ret_qty]"
                                                                    value="{{ $item->ret_qty }}"
                                                                    data-av_qty="tenant_return_{{ $key }}_av_qty" />
                                                            </td>
                                                            <td>
    ...
    <script>
        ...
        $(document).on('click', '.addRaw', function() {
            ...
            html = `<tr class="text-center">
                      <td>
                      <select class="form-control selectpicker text-center" id="tenant_return_` + i +
                `_warehouse_id" name="tenant_return[` + i + `][warehouse_id]" data-live-search = "true">
                      ...
                      <td>
                      <select class="form-control selectpicker category text-center" id="tenant_return_` + i +
                `_category_id" data-warehouse_id = "tenant_return_` + i +
                `_warehouse_id" data-product_id="tenant_return_` + i + `_product_id" data-live-search = "true">
                      <option value="">{{ __('Please Select') }}</option>
                      ...
                      <td>
                      <select class="form-control selectpicker product text-center" id="tenant_return_` + i +
                `_product_id" name="tenant_return[` + i + `][product_id]" data-warehouse_id = "tenant_return_` + i +
                `_warehouse_id" data-unit_show="tenant_return_` + i + `_unit_show" data-unit_id="tenant_return_` +
                i + `_unit_id" data-av_qty="tenant_return_` + i + `_av_qty" data-av_scale="tenant_return_` + i + `_av_scale" data-live-search = "true"></select>
                      </td>
                      <td><input class="form-control bg-primary text-center" id="tenant_return_` + i +
                `_unit_show" readonly/><input type="hidden" id="tenant_return_` + i + `_unit_id"/></td>
                      ...
                      <td><input class="form-control qty text-center" id="tenant_return_` + i +
                `_qty" name="tenant_return[` + i + `][qty]" data-product_id="tenant_return_` + i +
                `_product_id" data-unit_id="tenant_return_` + i + `_unit_id" data-av_qty="tenant_return_` + i +
                `_av_qty" data-av_scale="tenant_return_` + i + `_av_scale" data-scale="tenant_return_` + i + `_scale"/></td>
                      <td><input class="form-control scale text-center" id="tenant_return_` + i +
                `_scale" name="tenant_return[` + i + `][scale]" data-product_id="tenant_return_` + i +
                `_product_id" data-unit_id="tenant_return_` + i + `_unit_id" data-av_qty="tenant_return_` + i +
                `_av_qty" data-av_scale="tenant_return_` + i + `_av_scale" data-qty="tenant_return_` + i + `_qty"/> </td>
                      <td><input class="form-control retQty" id="tenant_return_` + i +
                `_ret_qty" name="tenant_return[` + i + `][ret_qty]" data-av_qty="tenant_return_` + i + `_av_qty"/></td>
                      <td>
                ...
                success: function(data) {
                    ...
                    if (data.status == false) {
                        $.each(data.errors, function(key, value) {
                            ...
                            $('#tenant_return_product_form select#' + key).parent().addClass(
                                'is-invalid');
                            $('#tenant_return_product_form #' + key).parent().append(
                                '<small class="error text-danger">' + value + '</small>');
                        });
                    } else {
                        notification(data.status, data.message);
                        if (data.status == 'success') {
                            window.location.replace("{{ route('tenant.return.product') }}");
                        }
                    }
                },
