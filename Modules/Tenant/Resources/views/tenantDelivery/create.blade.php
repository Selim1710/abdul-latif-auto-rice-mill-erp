                        ...
                        <form action="" id="tenant_delivery_product_form" method="post" enctype="multipart/form-data">
                            ...
                            <div class="row">
                                ...
                                <div class="col-md-12">
                                    <div class="form-group">
                                        <table class="table" id="tenantDeliveryTable">
                                            ...
                                            <tr class="text-center">
                                                <td>
                                                    <select class="form-control selectpicker text-center" id="tenant_delivery_0_warehouse_id" name="tenant_delivery[0][warehouse_id]" data-live-search = "true">
                                                ...
                                                <td>
                                                    <select class="form-control selectpicker category text-center" id="tenant_delivery_0_category_id" data-warehouse_id = "tenant_delivery_0_warehouse_id" data-product_id="tenant_delivery_0_product_id" data-live-search = "true">
                                                        <option value="">{{__('Please Select')}}</option>
                                ...
                                <div class="form-group col-md-12 text-center pt-5">
                                    <a class = "btn btn-danger btn-sm mr-3" href="{{route('tenant.delivery.product.add')}}"><i class="fas fa-sync-alt"></i>{{__('file.Reset')}}</a>
...
@push('scripts')
    <script>
        ...
        $(document).on('change','.product',function(){
            ...
            if(warehouseId != null && tenant_id != null){
                $.ajax({
                    ...
                    data    : {warehouseId : warehouseId , productId : productId , tenant_id : tenant_id},
                    method  : 'GET',
                    success : function(data){
                        if(data){
        ...
        function storeData(){
            ...
            let url      = "{{route('tenant.delivery.product.store')}}";
            $.ajax({
                url           : url,
                type          : "POST",
                data          : formData,
                ...
                success       : function (data) {
                    ...
                    if (data.status == false) {
                        ...
                        if (data.status == 'success') { window.location.replace("{{ route('tenant.delivery.product') }}"); }
                    }
                },
