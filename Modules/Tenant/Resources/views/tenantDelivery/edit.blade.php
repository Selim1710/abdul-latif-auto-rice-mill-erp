    ...
    <div class="container-fluid">
        <div class="card card-custom gutter-b">
            <div class="card-header flex-wrap py-5">
                ...
                <div class="card-toolbar"><a href="{{ route('tenant.delivery.product') }}" class="btn btn-warning btn-sm font-weight-bolder"><i class="fas fa-arrow-left"></i> {{__('file.Back')}}</a></div>
                @endif
            </div>
        </div>
                    ...
                    <form action="" id="tenant_delivery_product_form" method="post" enctype="multipart/form-data">
                        ...
                        <div class="row">
                            ...
                            <div class="form-group col-md-4 required">
                                ...
                                <input type="date" class="form-control date" id="date" name="date" value="{{$tenantDelivery->date}}" />
                            </div>
                            <div class="form-group col-md-4 required">
                                ...
                                <select class="form-control selectpicker" id="tenant_id" name="tenant_id" data-live-search="true">
                                    ...
                                    <option value="{{$tenant->id}}" @if($tenant->id == $tenantDelivery->tenant_id) selected="selected" @endif>{{$tenant->name.'('.$tenant->mobile.')'}}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="col-md-12">
                                ...
                                <div class="form-group">
                                    <table class="table" id="tenantDeliveryTable">
                                        ...
                                        <tbody>
                                            ...
                                            <tr class="text-center">
                                                <td>
                                                    <select class="form-control selectpicker text-center" id="tenant_delivery_{{$key}}_warehouse_id" name="tenant_delivery[{{$key}}][warehouse_id]" data-live-search="true">
                                                        <option value="{{$item->warehouse_id}}">{{$item->warehouse->name}}</option>
                                                    </select>
                                                </td>
                                                <td>
                                                    <select class="form-control selectpicker category text-center" id="tenant_delivery_{{$key}}_category_id" data-warehouse_id="tenant_delivery_{{$key}}_warehouse_id" data-product_id="tenant_delivery_{{$key}}_product_id" data-live-search="true">
                                                        <option value="{{$item->product->category_id}}">{{$item->product->category->category_name}}</option>
                                                    </select>
                                                </td>
                                                <td>
                                                    <select class="form-control selectpicker product text-center" id="tenant_delivery_{{$key}}_product_id" name="tenant_delivery[{{$key}}][product_id]" data-warehouse_id="tenant_delivery_{{$key}}_warehouse_id" data-unit_show="tenant_delivery_{{$key}}_unit_show" data-unit_id="tenant_delivery_{{$key}}_unit_id" data-av_qty="tenant_delivery_{{$key}}_av_qty" data-av_scale="tenant_delivery_{{$key}}_av_scale" data-live-search="true">
...
@push('scripts')
<script>
    let i = {{ count($tenantDelivery->tenantDeliveryProductList)}} + 1;
    ...
    $(document).on('click', '.addRaw', function() {
        ...
        html = `<tr class="text-center">
                      <td>
                      <select class="form-control selectpicker text-center" id="tenant_delivery_` + i + `_warehouse_id" name="tenant_delivery[` + i + `][warehouse_id]" data-live-search = "true">
                      ...
                      <td>
                      <select class="form-control selectpicker category text-center" id="tenant_delivery_` + i + `_category_id" data-warehouse_id = "tenant_delivery_` + i + `_warehouse_id" data-product_id="tenant_delivery_` + i + `_product_id" data-live-search = "true">
                      <option value="">{{__('Please Select')}}</option>
        ...
        $('#tenantDeliveryTable tbody').append(html);
        $('.selectpicker').selectpicker('refresh');
        i++;
    });
    ...
    function updateData() {
        ...
        let url = "{{route('tenant.delivery.product.update')}}";
        $.ajax({
            url: url,
            type: "POST",
            data: formData,
            ...
            success: function(data) {
                ...
                if (data.status == false) {
                    $.each(data.errors, function(key, value) {
                        ...
                        $('#tenant_delivery_product_form select#' + key).parent().addClass('is-invalid');
                        $('#tenant_delivery_product_form #' + key).parent().append('<small class="error text-danger">' + value + '</small>');
                    });
                } else {
                    notification(data.status, data.message);
                    if (data.status == 'success') {
                        window.location.replace("{{ route('tenant.delivery.product') }}");
                    }
                }
            },
