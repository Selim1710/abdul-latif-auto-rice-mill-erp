        ...
        <div class="container-fluid">
            <div class="card card-custom gutter-b">
                <div class="card-header flex-wrap py-5">
                    ...
                    <div class="card-toolbar"><a href="{{ route('distribution.add') }}"  class="btn btn-primary btn-sm font-weight-bolder"><i class="fas fa-plus-circle"></i>{{__('file.Add New')}}</a></div>
                    @endif
                </div>
            </div>
            <div class="card card-custom">
                <div class="card-header flex-wrap py-5">
                    <form method="POST" id="form-filter" class="col-md-12 px-0">
                        <div class="row">
                            <x-form.textbox labelName="{{__('file.Receiver Name')}}" name="receiver_name" col="col-md-4" />
                            <div class="col-md-4">
    ...
    <script>
        ...
        $(document).ready(function(){
            table = $('#dataTable').DataTable({
                ...
                "ajax": {
                    "url": "{{route('distribution.datatable.data')}}",
                    "type": "POST",
                    "data": function (data) {
                        data.receiver_name   = $("#form-filter #receiver_name").val();
                        data._token          = _token;
                    }
                },
                "columnDefs": [{
            ...
            $(document).on('click', '.edit_data', function () {
                ...
                $('#store_or_update_form #category_name').val($(this).data('category_name'));
                $('#store_or_update_form #type').val($(this).data('type'));
            ...
            $(document).on('click', '.change_status', function () {
                ...
                let url    = "{{ route('distribution.change.status') }}";
            ...
            $(document).on('click', '.delete_data', function () {
                ...
                let name  = $(this).data('name');
                let row   = table.row($(this).parent('tr'));
                let url   = "{{ route('distribution.delete') }}";
