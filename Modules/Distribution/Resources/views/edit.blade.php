                ...
                <div class="card-body">
                    <div id="kt_datatable_wrapper" class="dataTables_wrapper dt-bootstrap4 no-footer">
                        <form action="" id="distribution_store_form" method="post" enctype="multipart/form-data">
                            @csrf
                            <input type="hidden" name="update_id" value="{{$distribution->id}}"/>
                            <div class="row">
                                ...
                                <div class="form-group col-md-4 required">
                                    ...
                                    <input type="date" class="form-control text-center" id="date" name="date" value="{{$distribution->date}}"/>
                                </div>
                                <div class="form-group col-md-4 required">
                                    <label for="receiver">{{__('file.Receiver')}}.</label>
                                    <input type="text" class="form-control text-center" id="receiver_name" name="receiver_name" value="{{$distribution->receiver_name}}"/>
                                </div>
                                <div class="col-md-12">
                                    ...
                                    <div class="form-group">
                                        <div class="">
                                            <table class="table table-bordered distributionTableAppend" id="distributionTableAppend">
                                                ...
                                                <tbody>
                                                @if (!$distribution->distributionProductList->isEmpty())
                                                @foreach ($distribution->distributionProductList as $key => $item)
                                                <tr class="text-center">
                                                   <td>
                                                     <select class="form-control selectpicker" id="distribution_{{$key}}_warehouse_id" name="distribution[{{$key}}][warehouse_id]" data-product_id = "distribution_{{$key}}_product_id" data-live-search="true">
                                                         <option value = "{{$item->warehouse_id}}">{{$item->warehouse->name}}</option>
                                                     </select>
                                                   </td>
                                                   <td>
                                                      <select class="form-control selectpicker category" id="distribution_{{$key}}_category_id" data-warehouse_id="distribution_{{$key}}_warehouse_id" data-product_id = "distribution_{{$key}}_product_id" data-live-search="true">
                                                          <option value = "{{$item->product->category_id}}">{{$item->product->category->category_name}}</option>
                                                      </select>
                                                   </td>
                                                   <td>
                                                       <select class="form-control product selectpicker" id="distribution_{{$key}}_product_id" data-warehouse_id="distribution_{{$key}}_warehouse_id" data-unit_id_show = "distribution_{{$key}}_unit_id_show" data-unit_id = "distribution_{{$key}}_unit_id" data-available_qty = "distribution_{{$key}}_available_qty" name="distribution[{{$key}}][product_id]" data-live-search="true">
...
@push('scripts')
    <script>
        let i = {{count($distribution->distributionProductList)}} + 1;
        function _(x){
        ...
        function updateData(){
            ...
            let url      = "{{route('distribution.update')}}";
            $.ajax({
                url           : url,
                type          : "POST",
                data          : formData,
