                ...
                <div class="card-header flex-wrap py-5">
                    ...
                    <div class="card-toolbar">
                        <div>
                            ...
                            <span><a href="{{route('production.sale.add',$production->id)}}" class="btn btn-danger btn-sm font-weight-bolder"><i class="fab fa-opencart"></i>&nbsp;{{__('file.Sale')}}</a></span>
                            @endif
                            @if(permission('production-product-add'))
                            <span><a href="{{route('production.product.add',$production->id)}}" class="btn btn-info btn-sm font-weight-bolder"><i class="fas fa-boxes"></i>&nbsp;{{__('file.Stock')}}</a></span>
                            @endif
                            @if(permission('production-show'))
                            <span><a href="{{route('production.show',$production->id)}}" class="btn btn-primary btn-sm font-weight-bolder"><i class="fas fa-eye"></i> {{__('file.Details')}}</a></span>
                            @endif
                            @if(permission('production-access'))
                            <span><a href="{{route('production')}}" class="btn btn-secondary btn-sm font-weight-bolder"><i class="fas fa-arrow-circle-left"></i> {{__('file.Back')}}</a></span>
                            @endif
                        </div>
                    </div>
                </div>
...
@push('scripts')
    ...
    <script type="text/javascript">
        ...
        $(document).on('click','.next_button',function(){
            ...
            if(type == 'milling'){
                for(let i = 0 ; i < $('.rate').length ; i++){
                    if(__('useQty')[i].value == '' || __('useScale')[i].value == '' || __('useProQty')[i].value == '' || __('rate')[i].value == ''){
        ...
        function calculation(){
            ...
            _('per_unit_scale_cost').value = (+ productionRawAmount + + _('total_use_product_amount').value + + milling + + expenseCost) / ( + _('total_sale_scale').value + + _('total_stock_scale').value);
        }
