                        ...
                        <form action="" id="purchase_update_form" method="post" enctype="multipart/form-data">
                            ...
                            <div class="row">
                                ...
                                <div class="form-group col-md-3 required">
                                    ...
                                    <input type="date" class="form-control date" id="purchase_date" name="purchase_date" value="{{$edit->purchase_date}}"/>
                                </div>
                                <div class="col-md-3 form-group">
                                    ...
                                    <select class="form-control selectpicker" id="purchase_status" name="purchase_status" data-live-search = "true">
                                        ...
                                        @foreach(PURCHASE_STATUS_VALUE as $key => $value)
                                            <option value="{{$key}}" @if($key == $edit->purchase_status) selected="selected" @endif>{{$value}}</option>
                                        @endforeach
                                    </select>
                                </div>
                                ...
                                <div class="col-md-12">
                                    <div class="form-group">
                                        <div id="purchaseTableAppend" class="table-responsive">
                                            @if(!$edit->purchaseProductList->isEmpty())
                                            @foreach($edit->purchaseProductList as $key => $product)
                                            <table class="table">
                                                <tbody>
                                                ...
                                                <tr class="text-center">
                                                    <td>
                                                        <select class="form-control selectpicker text-center" id="purchase_{{$key}}_warehouse_id" name="purchase[{{$key}}][warehouse_id]" data-live-search = "true">
                                                            <option value="{{$product->warehouse_id}}">{{$product->warehouse->name}}</option>
                                                        </select>
                                                    </td>
                                                    <td>
                                                        <select class="form-control selectpicker category text-center" id="purchase_{{$key}}_category_id" data-product_id="purchase_{{$key}}_product_id" data-live-search = "true">
                                                            <option value="{{$product->product->category_id}}">{{$product->product->category->category_name}}</option>
                                                        </select>
                                                    </td>
                                                    <td>
                                                        <select class="form-control selectpicker product text-center" id="purchase_{{$key}}_product_id" name="purchase[{{$key}}][product_id]" data-unit_show="purchase_{{$key}}_unit_show" data-unit_id="purchase_{{$key}}_unit_id" data-price="purchase_{{$key}}_price" data-live-search = "true">
                                                ...
                                                <tr class="text-center">
                                                    ...
                                                    <th colspan="5"><button class="btn btn-primary btn-block">{{__('file.Note')}}</button></th>
                                                </tr>
                                                <tr>
...
@push('scripts')
    <script>
        @if(isset($edit)) partyType(); partyDue(); paymentMethod(); paymentStatus(); calculation();@endif
        let i = {{count($edit->purchaseProductList)}} + 1;
        function _(x){
                ...
                success       : function (data) {
                    ...
                    if (data.status == false) {
                        $.each(data.errors, function (key, value) {
                            ...
                            $('#purchase_update_form #' + key).parent().append('<small class="error text-danger">' + value + '</small>');
                        });
                    } else {
