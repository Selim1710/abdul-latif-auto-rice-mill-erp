...
//startcoa
                    ...
                    $employee_coa_data = $this->employeeCoa($head_name, $employees->id);
                    $employee_coa = ChartOfHead::create($employee_coa_data);
//endcoa
        ...
        if ($request->ajax()) {
            if (permission('employee-edit')) {
                ...
                try {
                    ...
                    //coa
                    $employee_coa = ChartOfHead::where(['employee_id' => $request->update_id])->first();
                    if ($employee_coa) {
                        $employee_coa->update(['name' => $request->name]);
                    }
    ...
    private function employeeCoa(string $head_name, int $employee_id)
    {
        return [
            'master_head' => 3,
            'type' => 3,
