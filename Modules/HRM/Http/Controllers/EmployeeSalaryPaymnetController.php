...
class EmployeeSalaryPaymnetController extends BaseController
{
    ...
    public function getEmployees(Request $request)
    {
        if ($request->ajax()) {
            if (permission('employee-salary-payment-add')) {
                $idArray = $request->id;
                $employees = DB::table('employees as e')->join('chart_of_heads as coa', 'coa.employee_id', 'e.id')->join('transactions as t', 't.chart_of_head_id', 'coa.id');
                if (!in_array("0", $idArray)) {
                    $employees->whereIn('e.id', $idArray);
                }
                $employees = $employees->groupBy('t.chart_of_head_id')->select(DB::raw('sum(debit) as debit'), DB::raw('sum(credit) as credit'), 'e.id as employee_id', 'e.name as name')->get();
    ...
    public function edit($id)
    {
        if (permission('employee-salary-payment-edit')) {
            ...
            $due = Transaction::where('chart_of_head_id', $coahead)->select(DB::raw('sum(debit) as debit'), DB::raw('sum(credit) as credit'))->first();
    ...
    public function approve($id)
    {
        if (permission('employee-salary-payment-change-status')) {
            ...
            try {
                ...
                $employeeCoa = ChartOfHead::where('employee_id', $data->employee_id)->first();
