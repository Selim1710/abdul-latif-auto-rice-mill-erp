        ...
        <div class="container-fluid">
            <div class="card card-custom gutter-b">
                <div class="card-header flex-wrap py-5">
                    ...
                    <div class="card-toolbar"><a href="{{ route('stock.transfer') }}"
                    ...
                    <form id="transfer_form" method="post" enctype="multipart/form-data">
                        ...
                        <div class="row">
                            <div class="col-md-3">
                                ...
                                <input type="text" class="form-control bg-primary text-white" name="invoice_no"
                                    value="{{ $invoiceNo }}" />
                            </div>
                            ...
                            <div class="col-md-3 form-group">
                                ...
                                <select class="form-control transfer_warehouse_id" id="transfer_warehouse_id"
                                    name="transfer_warehouse_id">
                        ...
                        <div class="row pt-5">
                            <div class="form-group col-md-12 text-center px-0">
                                <button type="button" class="btn btn-primary btn-sm mr-3" id="save-btn"
                                    onclick="transferData()"><i class="fas fa-save"></i>{{ 'Submit' }}</button>
                            </div>
                        </div>
                    </form>
                </div>
                    ...
                    if (data != '') {
                        $(data).each(function(index, item) {
                            html =
                                ...
                                <td><input type="text" class="form-control bg-primary text-white" id="transfer_` + i + `_party_name" value="` + item.party_name +`" readonly/><input type="hidden" id="transfer_` + i +
                                `_party_id" name="transfer[` + i +`][party_id]" value = "` + item.party_id +`"/>
                                </td>
                                <td>
                                    <input type="text" class="form-control bg-primary text-white" id="transfer_` + i + `_invoice_no" value="` + item.invoice_no +`" readonly/>
                                    <input type="hidden" id="transfer_` + i + `_purchase_id" name="transfer[` + i +`][purchase_id]" value = "` + item.purchase_id +`"/>
                                    <input type="hidden" id="transfer_` + i + `_purchase_price" name="transfer[` + i +`][purchase_price]" value = "` + item.purchase_price +`"/>
                                </td>

                                <td><input type="text" class="form-control bg-primary text-white" id="transfer_` + i + `_product_name" value="` + item.productName +`" readonly/><input type="hidden" id="transfer_` + i +
                                `_product_id" name="transfer[` + i +`][product_id]" value = "` + item.productId +`"/>
                                </td>
                                <td><input type="text" class="form-control bg-primary text-white" id="transfer_` + i + `_unit_name" value="` + item.unitName +
                                `" readonly/></td>
                                       <td><input type="text" class="form-control bg-primary text-white" id="transfer_` + i + `_stock_scale" value="` + item.scale +
                                `" readonly/></td>
                                       <td><input type="text" class="form-control bg-primary text-white" id="transfer_` + i + `_stock_qty" value="` + item.qty + `" readonly/></td>
                                       <td><input type="text" class="form-control scale" id="transfer_` + i +
                                `_receive_scale" data-stock_scale="transfer_` + i +
                                `_stock_scale" name="transfer[` + i + `][scale]"/></td>
                                       <td><input type="text" class="form-control qty" id="transfer_` + i +
                                `_receive_qty" data-stock_qty="transfer_` + i +
                                `_stock_qty" name="transfer[` + i + `][qty]"/></td>
                                    </tr>`
                ...
                success: function(data) {
                    ...
                    if (data.status == false) {
                        ...
                        if (data.status == 'success') {
                            window.location.replace("{{ route('stock.transfer') }}");
                        }
                    }
                },
