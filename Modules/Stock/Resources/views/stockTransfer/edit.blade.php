                                ...
                                <select class="form-control transfer_warehouse_id" id="transfer_warehouse_view">
                                    ...
                                    @foreach ($warehouses as $value)
                                        <option value = "{{ $value->id }}"
                                            @if ($value->id == $edit->transfer_warehouse_id) selected="selected" @endif>
                                            {{ $value->name }}</option>
                                    @endforeach
                                </select>
                                <input type="hidden" id="transfer_warehouse_id" name="transfer_warehouse_id"
                                    value = "{{ $edit->transfer_warehouse_id }}" />
                            </div>
                            <div class="col-md-4 form-group">
                                ...
                                <select class="form-control" id="receive_warehouse_id" name="receive_warehouse_id">
                                    ...
                                    @foreach ($warehouses as $value)
                                        <option value = "{{ $value->id }}"
                                            @if ($value->id == $edit->receive_warehouse_id) selected="selected" @endif>
                                            {{ $value->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                                        ...
                                        @foreach ($edit->stockTransferWarehouseProductList as $key => $value)
                                            <tr class = "text-center">
                                                <td><input type="text" class="form-control bg-primary text-white"
                                                        id="transfer_{{ $key }}_party_name"
                                                        value="{{ $value->party->name ?? '' }}" readonly /><input
                                                        type="hidden" id="transfer_{{ $key }}_party_id"
                                                        name="transfer[{{ $key }}][party_id]"
                                                        value = "{{ $value->party_id ?? '' }}" />
                                                </td>
                                                <td>
                                                    <input type="text" class="form-control bg-primary text-white"
                                                        id="transfer_{{ $key }}_invoice_no"
                                                        value="{{ $value->purchase->invoice_no ?? '' }}" readonly />
                                                    <input type="hidden" id="transfer_{{ $key }}_purchase_id"
                                                        name="transfer[{{ $key }}][purchase_id]"
                                                        value = "{{ $value->purchase_id ?? '' }}" />
                                                    <input type="hidden" id="transfer_{{ $key }}_purchase_price"
                                                        name="transfer[{{ $key }}][purchase_price]"
                                                        value = "{{ $value->purchase_price ?? '' }}" />
                                                </td>

                                                <td><input type="text" class="form-control bg-primary text-white"
                                                        id="transfer_{{ $key }}_product_name"
                                                        value="{{ $value->product->product_name }}" readonly /><input
                                                        type="hidden" id="transfer_{{ $key }}_product_id"
                                                        name="transfer[{{ $key }}][product_id]"
                                                        value = "{{ $value->product_id }}" /></td>
                                                <td><input type="text" class="form-control bg-primary text-white"
                                                        id="transfer_{{ $key }}_unit_name"
                                                        value="{{ $value->product->unit->unit_name . '(' . $value->product->unit->unit_code . ')' }}"
                                                        readonly /></td>
                                                <td><input type="text" class="form-control bg-primary text-white"
                                                        id="transfer_{{ $key }}_stock_qty"
                                                        value="{{ $value->availableQty($value->party_id, $value->purchase_id, $edit->transfer_warehouse_id, $value->product_id)->scale ?? 0 }}"
                                                        readonly /></td>
                                                <td><input type="text" class="form-control bg-primary text-white"
                                                        id="transfer_{{ $key }}_stock_scale"
                                                        value="{{ $value->availableQty($value->party_id, $value->purchase_id, $edit->transfer_warehouse_id, $value->product_id)->qty ?? 0 }}"
                                                        readonly /></td>
                                                <td><input type="text" class="form-control scale"
                                                        id="transfer_{{ $key }}_receive_scale"
                                                        data-stock_scale="transfer_{{ $key }}_stock_scale"
                                                        name="transfer[{{ $key }}][scale]"
                                                        value="{{ $value->scale }}" /></td>
                                                <td><input type="text" class="form-control qty"
                                                        id="transfer_{{ $key }}_receive_qty"
                                                        data-stock_qty="transfer_{{ $key }}_stock_qty"
                                                        name="transfer[{{ $key }}][qty]"
                                                        value="{{ $value->qty }}" /></td>
                                            </tr>
                                        @endforeach
                                    </tbody>
