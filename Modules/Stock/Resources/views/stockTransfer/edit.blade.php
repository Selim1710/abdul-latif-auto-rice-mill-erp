                    ...
                    <form id="transfer_form" method="post" enctype="multipart/form-data">
                                    ...
                                    <tbody>
                                        @foreach($edit->stockTransferWarehouseProductList as $key => $value)
                                            <tr class = "text-center">
                                                ...
                                                <td><input type="text" class="form-control bg-primary text-white" id="transfer_{{$key}}_unit_name" value="{{$value->product->unit->unit_name.'('.$value->product->unit->unit_code.')'}}" readonly/></td>
                                                <td><input type="text" class="form-control bg-primary text-white" id="transfer_{{$key}}_stock_qty" value="{{$value->availableQty($edit->transfer_warehouse_id,$value->product_id)->scale ?? 0}}" readonly/></td>
                                                <td><input type="text" class="form-control bg-primary text-white" id="transfer_{{$key}}_stock_scale" value="{{$value->availableQty($edit->transfer_warehouse_id,$value->product_id)->qty ?? 0}}" readonly/></td>
                                                <td><input type="text" class="form-control scale" id="transfer_{{$key}}_receive_scale" data-stock_scale="transfer_{{$key}}_stock_scale" name="transfer[{{$key}}][scale]" value="{{$value->scale}}"/></td>
                                                <td><input type="text" class="form-control qty" id="transfer_{{$key}}_receive_qty" data-stock_qty="transfer_{{$key}}_stock_qty" name="transfer[{{$key}}][qty]" value="{{$value->qty}}"/></td>
                                            </tr>
                                    @endforeach
                                    </tbody>
                                </table>
                        ...
                        <div class="row pt-5">
                            <div class="form-group col-md-12 text-center px-0">
                                <button type="button" class="btn btn-primary btn-sm mr-3" id="save-btn" onclick="transferUpdateData()"><i class="fas fa-save"></i>{{'Update'}}</button>
                            </div>
                        </div>
                    </form>
                </div>
