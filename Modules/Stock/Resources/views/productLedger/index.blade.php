        ...
        <div class="container-fluid">
            <div class="card card-custom gutter-b">
                <div class="card-header flex-wrap py-5">
                    ...
                    <div class="card-toolbar"><button type="button" class="btn btn-primary btn-sm mr-3" id="print-invoice"> <i class="fas fa-print"></i>{{__('file.Print')}}</button></div>
                </div>
            </div>
                ...
                <div class="card-header flex-wrap py-5">
                    <form method="POST" id="form-filter" class="col-md-12 px-0">
                        <div class="row">
                            ...
                            <x-form.selectbox labelName="{{__('file.Company')}}" name="warehouse_id" col="col-md-3" class="selectpicker">
                                @foreach ($warehouses as $warehouse)
                                    <option value="{{ $warehouse->id }}">{{ $warehouse->name }}</option>
                                @endforeach
                            </x-form.selectbox>
                            <x-form.selectbox labelName="{{__('file.Product Ledger')}}" name="product_ledger" col="col-md-3" class="selectpicker">
                                @foreach (PRODUCT_LEDGER_TYPE as $key => $value )
...
@push('scripts')
    ...
    <script>
        ...
        function report_data() {
            ...
            let party_id       = $('#party_id').find(":selected").val();
            let warehouse_id   = $('#warehouse_id').find(":selected").val();
            let product_ledger = $('#product_ledger').find(":selected").val();
            if( _('start_date').value == '' || _('end_date').value == ''){
                notification('error','Please choose date!');
                return;
            }
            if(product_ledger == ''){
                notification('error','Please Choose Product Ledger Type')
                return;
            }
            $.ajax({
                url        : "{{ route('product.ledger.data') }}",
                type       : "POST",
                data       :
                    {
