        ...
        <div class="card card-custom gutter-b">
            <div class="card-header flex-wrap py-5">
                ...
                <div class="card-toolbar">
                    <!--begin::Button-->
                    @if (permission('asset-add'))
                    <a href="javascript:void(0);" onclick="showNewFormModal('{{__('file.Add New Asset')}}','{{__('file.Save')}}')" class="btn btn-primary btn-sm font-weight-bolder">
        ...
        <div class="card card-custom">
            <div class="card-header flex-wrap py-5">
                <form method="POST" id="form-filter" class="col-md-12 px-0">
                    <div class="row">
                        <x-form.textbox labelName="{{__('file.Asset Tag')}}" name="tag" col="col-md-3" />
                        <x-form.textbox labelName="{{__('file.Asset Name')}}" name="name" col="col-md-3" />
                        <x-form.selectbox labelName="{{__('file.Asset Type')}}" name="asset_type_id" col="col-md-3" class="selectpicker">
                            @if (!$types->isEmpty())
                        ...
                        <x-form.selectbox labelName="{{__('file.Asset Status')}}" name="asset_status" col="col-md-3" class="selectpicker">
                            ...
                            <table id="dataTable" class="table table-bordered table-hover">
                                <thead class="bg-primary">
                                    <tr>
                                        ...
                                        <th>{{__('file.SL')}}</th>
                                        <th>{{__('file.Image')}}</th>
                                        <th>{{__('file.Asset Tag')}}</th>
                                        <th>{{__('file.Asset Name')}}</th>
                                        <th>{{__('file.Asset Type')}}</th>
                                        <th>{{__('file.Purchase Date')}}</th>
                                        <th>{{__('file.Cost')}}</th>
                                        <th>{{__('file.Warranty')}}</th>
                                        <th>{{__('file.Asset User')}}</th>
                                        <th>{{__('file.Location')}}</th>
                                        <th>{{__('file.Asset Status')}}</th>
...
<script>
    ...
    $(document).ready(function(){
        ...
        table = $('#dataTable').DataTable({
            ...
            "ajax": {
                "url": "{{route('asset.datatable.data')}}",
                "type": "POST",
                "data": function (data) {
                    data.tag           = $("#form-filter #tag").val();
                    data.name          = $("#form-filter #name").val();
                    data.asset_type_id = $("#form-filter #asset_type_id option:selected").val();
                    data.asset_status  = $("#form-filter #asset_status option:selected").val();
                    data._token        = _token;
                }
            },
            "columnDefs": [{
            ...
            "buttons": [
                ...
                },
                {
                    ...
                    "exportOptions": {
                        @if (permission('asset-bulk-delete'))
                        columns: ':visible:not(:eq(0),:eq(2),:eq(13))'
                        @else
                        columns: ':visible:not(:eq(1),:eq(12))'
                        @endif
                    },
        ...
        $(document).on('click', '#save-btn', function () {
            ...
            let url = "{{route('asset.store.or.update')}}";
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        if(data.status == 'error'){
                            ...
                            $('#store_or_update_form #tag').val(data.tag);
                            $('#store_or_update_form #asset_type_id').val(data.asset_type_id);
                            $('#store_or_update_form #purchase_date').val(data.purchase_date);
                            $('#store_or_update_form #warranty').val(data.warranty);
                            $('#store_or_update_form #user').val(data.user);
                            $('#store_or_update_form #location').val(data.location);
                            $('#store_or_update_form #cost').val(data.cost);
                            $('#store_or_update_form #asset_status').val(data.asset_status);
                            ...
                            if(data.photo){
                                ...
                                $('#store_or_update_form .img_').attr('src',`{{ 'storage/'.ASSET_PHOTO_PATH }}`+data.photo);
                            }else{
                            ...
                            $('#store_or_update_form #old_photo').val(data.photo);
        ...
        function multi_delete(){
            ...
            if(ids.length == 0){
                ...
                let url = "{{route('asset.bulk.delete')}}";
        ...
        $(document).on('click', '.change_status', function () {
            ...
            let row    = table.row($(this).parent('tr'));
            let url    = "{{ route('asset.change.status') }}";
    ...
    function showNewFormModal(modal_title, btn_text) {
        ...
        $('#store_or_update_form .img_').css('display','none');
        $('#store_or_update_form .img_').attr('src','');
        $('#store_or_update_form #old_photo').val('');
