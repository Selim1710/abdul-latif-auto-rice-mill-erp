...
@section('content')
            ...
            <div class="card card-custom gutter-b">
                <div class="card-header flex-wrap py-5">
                    ...
                    <div class="card-toolbar">
                        @if (permission('loan-categories-store-update'))
                            <a href="javascript:void(0);" onclick="showFormModal('{{__('file.Add New Name')}}','{{__('file.Save')}}')" class="btn btn-primary btn-sm font-weight-bolder"><i class="fas fa-plus-circle"></i> {{__('file.Add New')}}</a>
                        @endif
                    </div>
                </div>
            </div>
    ...
    @include('loan::loanCategories.modal')
@endsection
@push('scripts')
    <script>
        let table;
        $(document).ready(function(){
            table = $('#dataTable').DataTable({
                ...
                "ajax"    : {
                    "url" : "{{route('loan.categories.datatable.data')}}",
                    "type": "POST",
                    "data": function (data) {
                        data._token         = _token;
                    }
            ...
            $(document).on('click', '#save-btn', function () {
                ...
                let url      = "{{route('loan.categories.store.or.update')}}";
            ...
            $(document).on('click', '.edit_data', function () {
                ...
                if (id) {
                    $.ajax({
                        url     : "{{route('loan.categories.edit')}}",
                        type    : "POST",
            ...
            $(document).on('click', '.delete_data', function () {
                ...
                let url   = "{{ route('loan.categories.delete') }}";
